	...
	public boolean onCommand(final CommandSender sen, final Command cmd, final String lab, final String[] args) {
		if (sen.hasPermission("helpy.ArrowsInBody")) {
			if (sen instanceof Player) {
				...
				if (args.length == 0) {
					Eng.atChange(p, "action", "§eArrows §7in §eBody", false, "" + p.getArrowsInBody(), "0", true, false);
					p.setArrowsInBody(0);
					...
					try {
						...
						Eng.atChange(p, "action", "§eArrows §7in §eBody", false, "" + p.getArrowsInBody(), "" + a, true, false);
						p.setArrowsInBody(a);
						...
						Eng.atChange(t, "action", "§eArrows §7in §eBody", false, "" + t.getArrowsInBody(), "0", true, false);
				...
				if (t2 != null) {
					try {
						...
						Eng.atChange(t2, "chat", "§eArrows §7in §eBody", false, "" + t2.getArrowsInBody(), "" + a, true, false);
