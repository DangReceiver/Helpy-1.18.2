...
public class TreeCutDown implements Listener {
    ...
    public void onTreeFarm(final BlockBreakEvent e) {
        ...
        if (p.isSneaking() || p.getGameMode() == GameMode.SURVIVAL || p.getGameMode() == GameMode.ADVENTURE) {
            ...
            if (itemHand.getType().toString().contains("_AXE") &&
                    e.getBlock().getType().toString().contains("LOG")) {
                short d = 1;
                for (int times = 0; times <= 48; times++) {
                    int eTimes = times;
                    d = (short) (d + 2);
                    if (e.getBlock().getLocation().add(0.0D, (eTimes + 1), 0.0D).getBlock().getType().equals(b.getType())) {
